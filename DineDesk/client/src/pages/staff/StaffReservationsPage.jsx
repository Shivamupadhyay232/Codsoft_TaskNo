import { CalendarCheck, CheckCircle2, Clock, Filter, RefreshCw, XCircle } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200">
        <div>
          <h1 className="text-2xl font-bold font-serif text-slate-900">Table Reservations Desk</h1>
          <p className="text-xs text-slate-500">Manage table allocations and guest booking statuses</p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="No bookings match the selected date and status filters."
        />
      ) : (
                        ...
                        <div className="flex items-center justify-end gap-1.5">
                          {resv.status === 'PENDING' && (
                            <>
                              <Button
                                ...
                                onClick={() => handleStatusChange(resv.id, 'CONFIRMED')}
                              >
                                Confirm
                              </Button>
                              <Button
                                ...
                                onClick={() => handleStatusChange(resv.id, 'CANCELLED')}
                              >
                                Reject
                              </Button>
                            </>
                          ...
                          {resv.status === 'CONFIRMED' && (
                            <Button
                              variant="outline"
                              size="sm"
                              className="text-[11px] py-1 px-2.5 text-emerald-600 border-emerald-200 hover:bg-emerald-50"
                              onClick={() => handleStatusChange(resv.id, 'COMPLETED')}
                            >
                              Seat / Complete
                            </Button>
                          )}
